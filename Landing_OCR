...
The operator is deployed with the provided installation script, `kubernetes/create-weblogic-operator.sh`. The input to this script is the file `kubernetes/create-operator-inputs.yaml`, which needs to be updated to reflect the target environment.
...
You can choose a load balancer provider for your WebLogic domains running in a Kubernetes cluster. Please refer to [Load balancing with Voyager/HAProxy](https://github.com/oracle/weblogic-kubernetes-operator/site/voyager.md), [Load balancing with Traefik](https://github.com/oracle/weblogic-kubernetes-operator/site/traefik.com), and [Load balancing with the Apache HTTP Server](https://github.com/oracle/weblogic-kubernetes-operator/site/apache.md) for information about the current capabilities and setup instructions for each of the supported load balancers.
